...
pval <- function(x) if_else(is.na(x), "", if_else(x < 0.001, "<0.001", sprintf("%.3f", x)))
...
#| message: false
# Use S8 as the source for precise category counts and labels
burden_data <- read_csv("data/S8_ordinal_sensitivity_category_counts.csv", show_col_types = FALSE)

burden_data |>
  transmute(
    `Burden Level` = case_when(
      str_detect(syndemic_sensitivity_ord, "Low") ~ "Low Burden",
      str_detect(syndemic_sensitivity_ord, "Medium") ~ "Medium Burden",
      str_detect(syndemic_sensitivity_ord, "High") ~ "High Burden"
    ),
    `Score Range` = case_when(
      `Burden Level` == "Low Burden" ~ "0–1",
      `Burden Level` == "Medium Burden" ~ "2–3",
      `Burden Level` == "High Burden" ~ "≥4"
    ),
    `Unweighted n` = format(unweighted_n, big.mark = ","),
    `Weighted Percentage (%)` = sprintf("%.1f%%", percent)
  ) |>
  kable(
...
### S4. Ordinal Sensitivity Analysis — Burden Category Counts

```{r}
#| label: s3-ordinal-counts
s8 <- read_csv("data/S8_ordinal_sensitivity_category_counts.csv", show_col_types = FALSE)

s8 |>
  kable(caption = "S4. Syndemic burden category counts for ordinal sensitivity analysis", align = "lrr")
```

### S5. Ordinal Sensitivity Analysis — Full Results

The ordinal proportional-odds model uses the same predictors as the primary quasi-Poisson model. Results are odds ratios (OR) for being in a higher burden category. The ordinal model is supplementary; its primary role here is to confirm the direction and significance of predictors found in the quasi-Poisson model.

```{r}
#| label: s4-ordinal-results
s9 <- read_csv("data/S9_ordinal_sensitivity_results.csv", show_col_types = FALSE)
...
s9 |>
  ...
  transmute(
    ...
    `OR (95% CI)` = OR_CI,
    `p-value`   = p_formatted,
    Note        = sensitivity_role
  ) |>
  kable(caption = "S5. Ordinal logistic sensitivity analysis: odds ratios for elevated syndemic burden", align = "lllll")
```

### S6. Missingness Summary
...
missingness |>
  mutate(missing_percent = pct1(missing_percent)) |>
  kable(caption = "S6. Missingness summary for risk indicators and model covariates", align = "lrrr")
```

### S7. Available-Indicator vs Complete-Case Score Sensitivity
...
complete_case |>
  mutate(weighted_percent = pct1(weighted_percent)) |>
  kable(caption = "S7. Available-indicator vs complete 9-indicator burden distribution", align = "llrl")
```

### S8. Very High Burden by Division (score ≥6)
...
very_high_division |>
  ...
  kable(caption = "S8. Very high syndemic burden (score ≥6) by division", align = "ll")
```

### Additional Figures

![Wealth-residence intersectionality.](figures/figure5_intersectionality_wealth_residence.jpg){fig-alt="Intersectional differences in syndemic burden by wealth quintile and urban-rural residence."}
